<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'snacks' => "`snacks`.`ID` as 'ID', `snacks`.`brand` as 'brand', `snacks`.`category` as 'category', `snacks`.`flavor` as 'flavor', `snacks`.`item` as 'item', `snacks`.`Illustration` as 'Illustration', `snacks`.`serving_size` as 'serving_size', `snacks`.`calories` as 'calories', `snacks`.`fat` as 'fat', `snacks`.`carbs` as 'carbs', `snacks`.`protein` as 'protein', `snacks`.`fiber` as 'fiber', `snacks`.`sodium` as 'sodium', `snacks`.`am_link` as 'am_link', `snacks`.`name` as 'name', `snacks`.`rating` as 'rating', `snacks`.`starcount` as 'starcount', `snacks`.`favorite` as 'favorite'",
			'ratings' => "`ratings`.`id` as 'id', `ratings`.`member` as 'member', `ratings`.`snackid` as 'snackid', `ratings`.`rating` as 'rating'",
			'favorites' => "`favorites`.`id` as 'id', `favorites`.`member` as 'member', `favorites`.`snackid` as 'snackid'",
		];
	...
	function htmlUserBar() {
		...
		?>
		<nav style="display:none;" class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
